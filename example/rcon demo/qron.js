...
// const Rcon = require('./qron');

let data = {
    host: "localhost",
    rcon_port: 32330,
    rcon_password: "kvixFUSOaUEyDVZF",
    game: "zzzzzz",
    web: false
}

let conn = new Rcon('localhost', 32330, {
    serverData: data
});
conn.on("message", function (rconMessage) {
    console.log("message:" + rconMessage.body)
});
conn.connect(() => {
    console.log('链接成功')
    conn.send("DestroyAllEnemies", null, false, function (result) {

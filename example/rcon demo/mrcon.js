...
class RconCommand {
    constructor(host = "", port = 0, password = "", options = {}) {
        this.execCommand = "";
        const conn = this.conn = new Rcon(host, port, password, options);
        conn.on('auth', async () => {
            console.log("RCON connected");
            if (this.execCommand) {
                this.conn.send(this.execCommand);
                this.execCommand = "";
            }
        }).on('response', (str) => {
    ...
    disconnect() {
        ...
        this.conn._tcpSocket.end();
        this.conn._tcpSocket.destroy();
    }
...
module.exports = { RconCommand, Rcon };
